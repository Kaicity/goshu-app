...
const ChangePasswordContent = () => {
  ...
  const [state, submitAction, isPending] = useActionState(async (prevState: any, formData: loginFormData) => {
    try {
      ...
      if (res) {
        ...
        navigate.replace('/authentication');
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    if (token && account) {
      ...
      navigate.push('/authentication/forgot-password');
    }
  }, [searchParams]);
      ...
      <Card>
        <CardHeader>
          <>
            ...
            <CardDescription>Nhập mã code được gửi đến email và đổi mật khẩu mới</CardDescription>
          </>
        </CardHeader>
          ...
          >
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Button className="w-max" type="submit" disabled={resetPassword}>
                  {isLoading ? <Loader2 className="size-4 mr-2 animate-spin" /> : 'Xác nhận'}
                </Button>
              </div>
            </div>
          </form>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-5">
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                  <SubmitButton text="Đổi mật khẩu" isPending={isPending} className="w-full" />
                </div>
              </div>
            </form>
          )}
